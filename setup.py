...
def get_extensions():
    ...
    extensions_dir = os.path.join(this_dir, "DetectionHub", "csrc")
...
class rename_custom_ops_lib(distutils.command.build.build):
    def run(self):
        inst_dir = os.path.join(self.build_lib, 'DetectionHub', 'lib')
        lib_dir = os.path.join('DetectionHub', 'lib')
        ...
        os.rename(os.path.join(inst_dir, 'custom_ops.' + lib_suffix),
                  os.path.join(inst_dir, 'libDetectionHub_customops.' + ext))
        shutil.copy(os.path.join(inst_dir, 'libDetectionHub_customops.' + ext),
                    os.path.join(lib_dir, 'libDetectionHub_customops.' + ext))
...
setup(
    ...
    version="0.1",
    author="wdw",
    url="https://github.com/Miracle1991/DetectionHub",
