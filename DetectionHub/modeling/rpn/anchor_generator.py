...
class AnchorGenerator(nn.Module):
    ...
    def grid_anchors(self, grid_sizes):
        ...
        for size, stride, base_anchors in zip(
            ...
            # WORKAROUND: torch.arange export to ONNX not supported yet.
...
class AnchorGenerator_yolov3(nn.Module):
    """
    For a set of image sizes and feature maps, computes a set
    of anchors
    """
    ...
    def __init__(
        ...
        super(AnchorGenerator_yolov3, self).__init__()

        cell_anchors = generate_anchors_yolov3()
        self.strides = anchor_strides
        self.cell_anchors = BufferList(cell_anchors)
        self.straddle_thresh = straddle_thresh
...
def make_anchor_generator_yolov3(config):
    ...
    anchor_generator = AnchorGenerator_yolov3(anchor_strides, straddle_thresh)
    return anchor_generator
...
def generate_anchors_yolov3(
    anchors = [[[-5, -6.5, 5, 6.5],
               [-8, -15, 8, 15],
               [-16.5, -11.5, 16.5, 11.5]],
               [[-15, -30.5, 15,  30.5],
                [-31.5, -22.5, 31.5, 22.56],
                [-29.5, -59.5, 29.5,  59.5]],
                [[-58, -45, 58,  45],
                [-78, -99, 78,  99],
                [-373 / 2, -326 / 2, 373 / 2, 326 / 2]]]
        ):
    """Generates a matrix of anchor boxes in (x1, y1, x2, y2) format. Anchors
    are centered on stride / 2, have (approximate) sqrt areas of the specified
    sizes, and aspect ratios as given.
    ...
    # anchors[:, :, 1] = anchors[:, :, 2]
    # anchors[:, :, 2] = temp
    return torch.from_numpy(anchors).to(torch.float32)
