...
from DetectionHub.modeling.balanced_positive_negative_sampler import (
    BalancedPositiveNegativeSampler
)
from DetectionHub.modeling.utils import cat
...
class FastRCNNLossComputation(object):
    ...
    def __call__(self, class_logits, box_regression):
        ...
        labels_onehot = torch.zeros(labels.shape[0], 2).scatter_(1, labels, 1)

        labels_onehot = torch.where(labels_onehot > 0, labels_onehot - 0.3, labels_onehot + 0.3).cuda()
        classification_loss = F.binary_cross_entropy_with_logits(class_logits, labels_onehot)

        return classification_loss * 10, box_loss
