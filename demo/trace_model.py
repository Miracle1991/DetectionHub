...
from DetectionHub.structures.image_list import ImageList
...
def my_paste_mask(mask, bbox, height, width, threshold=0.5, padding=1, contour=True, rectangle=False):
    ...
    scaled_mask = torch.ops.DetectionHub.upsample_bilinear(padded_mask.float(), h, w)
...
def add_annotations(image, labels, scores, bboxes, class_names=','.join(coco_demo.CATEGORIES), color=torch.tensor([255, 255, 255], dtype=torch.long)):
    ...
    result_image = torch.ops.DetectionHub.add_annotations(image, labels, scores, bboxes, class_names, color)
    return result_image
